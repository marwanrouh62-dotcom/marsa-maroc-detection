...
| Enregistrement automatique du passage (`SuiviPassage`) | 4 tests unitaires (`test_passage_tracker.py`) : passage normal, détection isolée, image ratée au milieu, vote majoritaire | 4/4 réussis (voir `MANUEL_TECHNIQUE.md` §3sexies) |
| Écriture réelle d'un passage automatique (`_enregistrer_passage`) | appel direct avec une vraie image | entrée SQLite créée avec les bonnes valeurs, fichier image bien écrit dans `captures/`, vérifiés puis nettoyés |
